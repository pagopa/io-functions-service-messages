...
}) => ({ newRCConfiguration, configurationId }) =>
  pipe(
    ...
    TE.chain(() =>
      pipe(
        rccModel.upsert({ ...newRCConfiguration, configurationId }),
        TE.mapLeft(e =>
          ResponseErrorInternal(
...
export const getUpdateRCConfigurationExpressHandler: GetUpdateRCConfigurationHandler = ({
  ...
  return wrapRequestHandler(
    middlewaresWrap((_, configurationId, newRCConfiguration) =>
      handler({ configurationId, newRCConfiguration })
    )
  );
};
